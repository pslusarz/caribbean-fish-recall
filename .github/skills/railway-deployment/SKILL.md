...
**Status: not yet configured.** This app has not been connected to GitHub or
Railway yet — that's the deliberate pause point after local verification
(see planning/CURRENT-EPIC-LUSTERECZKO-MIGRATION-IN-PROGRESS.md). Do not
attempt to run `railway` CLI commands against a project/service name until
this section has been filled in with real values below, mirroring
barobeaver's version of this file.
...
1. Create a GitHub repository for this project and push the local git history.
...
4. Set up branch-based deploys (staging branch -> staging environment, main -> production), matching barobeaver's setup.
